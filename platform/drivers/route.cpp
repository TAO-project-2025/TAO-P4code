...
namespace shc
{
  ...
  void Route::DataSetUp(const hal_route_data &data,
    ...
    // printf("data.nhop_id = %u\n",data.nhop_id);                                    
    bf_sys_assert(bf_status == BF_SUCCESS);

    bf_status = table_data->setValue(set_route_for_nexthop_action_ttl_id_, entry_ttl);
    bf_sys_assert(bf_status == BF_SUCCESS);
    return;
  }
  ...
  Route::Route(const bfrt::BfRtInfo *bfrtInfo, bf_rt_target_t& d_tgt, std::shared_ptr<bfrt::BfRtSession>& common_session){
    ...
    bf_status = table_ptr_->keyFieldIdGet("ig_md.src_addr", &ip_src_field_id_); //对应到硬件表key字段
    bf_sys_assert(bf_status == BF_SUCCESS);

      bf_status = table_ptr_->keyFieldIdGet("ig_md.dst_addr", &ip_dst_field_id_); //对应到硬件表key字段
    bf_sys_assert(bf_status == BF_SUCCESS);
    ...
    bf_status =
        table_ptr_->dataFieldIdGet("$ENTRY_TTL",
                                    set_route_for_nexthop_action_id_,
                                    &set_route_for_nexthop_action_ttl_id_); //
    bf_sys_assert(bf_status == BF_SUCCESS);
  ...
  hal_status_t Route::tableEntryDel(const hal_route_key *key, bool aged_flag){
  ...
  void Route::aging_ttl_balance(){
    float entry_share = (float)entry_in_table_c / 65536;
    if(entry_share > 0.85){
        aged_replace = true;
    }
    float avaliable_share = (float)(65536 - entry_in_table_c + entry_aged_c) / 65536;
    if(aged_replace){
        if(avaliable_share < 0.15){
          if(entry_ttl>60){
            entry_ttl -= 5;
          }
        }
    ...
    // entry_ttl = 50;
    // printf("entry_in_table_c = %u, entry_aged_c = %u, aged_replace = %d\n",entry_in_table_c,entry_aged_c,aged_replace);
    // printf("entry_share = %f, avaliable_share = %f, entry_ttl = %lu\n",entry_share,avaliable_share,entry_ttl);
    return;
  }
