...
ol_ip_rcv(struct rte_mbuf *pkt, void *ctx)
{
    ...
    ret = ntb_route_lookup(vrf_dp, iphdr->dst_addr, &f_res,0); //check vrf(route) table find the exact route path
    if (ret != 0) {
...
vxlan_tunnel_rcv(struct rte_mbuf *pkt, void *ctx)
{
    ...
    vrf_dp = ntb_tunnel_map_vrf_lookup(NTB_TUNNEL_VXLAN, tunnel_id,0); //check tunnel map vrf table
    if (!vrf_dp) {
